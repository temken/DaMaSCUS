...
int main(int argc, char* argv[])
{
	...
	if(myRank == 0)
	{
		...
		cout << "\n##############################" << endl
			 << "DaMaSCUS" << version << " - Data Analysis" << endl
			 << endl
		...
		if(nError != 0)
		{
			cout << "Folder already exists, existing data will be overwritten." << endl
				 << endl;
		}
		else
		{
	...
	if(myRank == 0)
		cout << "Broadcast local DM densities to all MPI processes." << endl;
	...
	for(int i = iList[myRank]; i < iList[myRank + 1]; i++)
	{
		...
		for(int j = 0; j < bins; j++)
			f << VelocityHistogram[j][0] << "\t" << VelocityHistogram[j][1] << "\t" << VelocityHistogram[j][2] << "\t" << VelocityHistogram[j][3] << endl;
		f.close();
		...
		for(unsigned int j = 0; j < etaH.size(); j++)
			f << etaH[j][0] << "\t" << etaH[j][1] << "\t" << etaH[j][2] << "\t" << etaH[j][3] << endl;
		f.close();
		...
		else if(experiment == "LUX")
		{
			...
			f.open("../results/" + SimID + "_histograms/dRdE." + std::to_string(i));
			for(unsigned int j = 0; j < dRdEH.size(); j++)
				f << dRdEH[j][0] << "\t" << dRdEH[j][1] << "\t" << dRdEH[j][2] << "\t" << dRdEH[j][3] << "\t" << dRdErA(dRdEH[j][0], 1, 131, rhoDM, mChi, sigma0, vEarth.norm()) << endl;
			f.close();
			// Calculate Total event rate
			// Compute the analytic result at the first iteration
			if(i == iList[myRank])
				R_A = R_LUX_A(rhoDM, mChi, sigma0, vEarth.norm());
		...
		{
			if(myRank == 0)
				cout << "Error: Experiment not recognized. No detection rate will be computed." << endl;
			experiment = "None";
		}
	...
	if(myRank == 0)
	{
		...
		if(rc)
			cout << "Unable to read file " << SpeedFilename << "." << endl;
		// ASCII Output files
		ofstream f;
		...
		if(experiment != "None")
		{
			...
			if(rc)
				cout << "Unable to read file " << RateFilename << "." << endl;
			// ASCII Output files
			ofstream g;
